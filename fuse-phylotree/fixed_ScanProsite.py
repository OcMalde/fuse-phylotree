...
"""Code for calling and parsing ScanProsite from ExPASy (modified in accordance with the scanprosite url)."""
...
def scan(seq="", mirror="https://prosite.expasy.org", output="xml", **keywords):
    ...
    handle = urlopen(url, context=ssl.create_default_context(cafile=certifi.where()))
    return handle
