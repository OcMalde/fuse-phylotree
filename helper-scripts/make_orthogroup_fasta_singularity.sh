...
if [ "$#" -ne 4 ]; then
    echo "Usage: $0 <id_seq_file> <id_outgroup_file> <name> <path_image>"
    exit 1
fi
...
path_image="$4"                       # Path to the Singularity image

# Convert path_image to absolute if needed
workplace=$(pwd)
if [[ "${path_image}" != /* ]]; then
    path_image="${workplace}/${path_image}"
fi
...
# Run orthogroup FASTA creation for proteins of interest
...
# Run orthogroup FASTA creation for outgroup proteins
